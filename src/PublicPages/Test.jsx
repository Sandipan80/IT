import React, { Suspense, lazy } from "react";import { Button, Card, Tag, Carousel, Input, Space } from "antd";
  ...
  return (
    <div className="min-h-screen bg-white font-sans text-slate-900">
      ...
      <section className="relative h-screen flex flex-col items-center justify-center overflow-hidden bg-[#f8fafc]">
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-blue-100/50 rounded-full blur-[120px] -z-10" />
      ...
      <section className="py-24 px-6 max-w-7xl mx-auto">
        <div className="text-center mb-16">
          <h2 className="text-4xl font-bold mb-4">Driving Industry Innovation</h2>
          <p className="text-slate-500 max-w-2xl mx-auto">
            Synthesized results from global IT asset management and helpdesk platform case studies.
          </p>
        </div>
      ...
      <section className="bg-[#0a0c10] py-24 px-6 text-white">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16">
            <p className="text-blue-400 font-bold tracking-[0.3em] uppercase text-sm mb-4">What We Do</p>
            <h2 className="text-5xl font-black mb-12">Three pillars. <br />One platform.</h2>
          </div>
      ...
      <section className="py-24 px-6 bg-slate-50">
        <div className="max-w-7xl mx-auto">
          <div className="flex justify-between items-end mb-12">
            <div>
              <p className="text-blue-600 font-bold tracking-widest text-xs uppercase mb-2">Project Portfolio</p>
              <h2 className="text-6xl font-black leading-tight">Work that speaks <br /><span className="text-blue-500 font-serif italic">for itself.</span></h2>
            </div>
          </div>
...
const ImpactCard = ({ tag, title, desc, icon }) => (
  <motion.div variants={fadeInUp}>
    <Card className="h-full border-none shadow-xl shadow-slate-200/50 hover:shadow-2xl transition-shadow duration-300">
      ...
      <Tag color="blue" className="mb-4 border-none font-bold px-3 py-0.5 rounded-full">{tag}</Tag>
...
const FeatureCard = ({ icon, title, subtitle, content, list, color }) => {
  const accentClass = color === "blue" ? "text-blue-400" : color === "green" ? "text-green-400" : "text-purple-400";
  return (
    <div className="bg-[#14181f] p-8 rounded-3xl border border-white/5 hover:border-white/10 transition-all group">
      <div className="mb-8 p-4 bg-white/5 w-fit rounded-xl group-hover:scale-110 transition-transform">{icon}</div>
      ...
      <ul className="space-y-3">
        {list.map((item, i) => (
          <li key={i} className="flex items-center text-xs text-slate-300">
            <span className={`w-1.5 h-1.5 rounded-full mr-3 ${color === "blue" ? "bg-blue-500" : color === "green" ? "bg-green-500" : "bg-purple-500"}`} />
            {item}
          </li>
        ))}
      </ul>
...
const ProjectCard = ({ category, time, title, client, outcome }) => (
  <motion.div whileHover={{ y: -5 }} className="bg-white p-10 rounded-3xl shadow-sm border border-slate-100 flex flex-col h-full">
    <div className="flex justify-between items-start mb-6">
      <Tag className="bg-blue-50 text-blue-600 border-none px-4 py-1 rounded-full font-bold text-[10px] tracking-wider uppercase">{category}</Tag>
      <span className="text-slate-400 text-xs font-medium">{time}</span>
    </div>
    <h3 className="text-2xl font-bold mb-3">{title}</h3>
    <p className="text-slate-400 text-sm mb-8">{client}</p>
    <div className="mt-auto pt-8 border-t border-slate-50">
      <p className="text-slate-700 font-medium"><span className="font-bold text-slate-900">Outcome:</span> {outcome}</p>
      <div className="mt-6">
        <Tag color="success" className="rounded-full border-none px-4 py-0.5 font-bold text-[10px]">● DELIVERED</Tag>
      </div>
    </div>
  </motion.div>
);
