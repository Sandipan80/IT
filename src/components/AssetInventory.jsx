...
const api = {
  ...
  // Fetch employees to populate the assignment dropdown
  getEmployees: () =>
    fetch(`${EMP_BASE}/GetUser`)
      .then(r => r.json()),
...
export default function AssetInventory() {
  ...
  const [employees, setEmployees]           = useState([]); // State for assignment dropdown
  ...
  /**
   * INITIALIZATION
   * Runs on mount: Checks server health, then fetches assets and employees
   */
  useEffect(() => {
    (async () => {
      try {
        await api.health();
        setApiOnline(true);
        // Load both assets and employees simultaneously
  ...
  const fetchEmployees = async () => {
    try {
      ...
      if (res.employeeList) setEmployees(res.employeeList);
    } catch (err) {
  ...
  const openEditDrawer = (asset) => {
    ...
    form.setFieldsValue({
      ...asset,
      // CRITICAL: extract the ID if assignedTo is a populated object
      assignedTo: typeof asset.assignedTo === 'object' ? asset.assignedTo?._id : asset.assignedTo,
    });
    setDrawerOpen(true);
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (emp, record) => {
        // FIX: 'emp' is now an object because of .populate().
        // We check if it is an object and extract the 'Name' property.
        if (record.status === "Assigned" && emp) {
          const displayName = typeof emp === 'object' ? emp.Name : emp;
          return (
            <div className="flex items-center gap-1.5">
              <div className="w-6 h-6 rounded-full bg-indigo-100 flex items-center justify-center shrink-0">
                <UserOutlined className="text-indigo-600 text-xs" />
              </div>
              <span className="text-slate-700 text-sm">{displayName}</span>
            </div>
          );
        }
    ...
    <div className="min-h-screen bg-linear-to-br from-slate-50 via-indigo-50/30 to-purple-50/20 font-sans">
      ...
      <div className="bg-white border-b border-slate-200 shadow-sm sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-linear-to-br from-indigo-600 to-purple-700 flex items-center justify-center shadow-lg">
          ...
          {[
            ...
            <Col xs={12} sm={6} key={s.label}>
              <div className="bg-white rounded-2xl p-4 shadow-sm border border-slate-100 flex items-center gap-4 hover:shadow-md transition-shadow">
                <div className={`w-12 h-12 rounded-xl bg-linear-to-br ${s.color} flex items-center justify-center text-white text-xl shadow shrink-0`}>
                  {s.icon}
                </div>
                <div>
      ...
      >
        <Form form={form} layout="vertical">
          ...
          {/* ASYNC DROPDOWN FOR FLEXIBLE ASSIGNMENT */}
